  ...
  >
    ...
    <div class="quote-back" ref="backEl">
      <div class="icons-list">
        <svg class="icon restart" width="27" height="29" viewBox="0 0 27 29" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="icon-wrap play-pause" style="margin-left: -2px">
          <svg class="icon pause toggled" width="13" height="21" viewBox="0 0 13 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="3.21154" height="20.875" fill="white"/>
            <rect x="9.63464" width="3.21154" height="20.875" fill="white"/>
          </svg>

          <svg class="icon play secondary" width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M17.25 9.95929L9.69333e-07 19.9186L1.84e-06 -3.61504e-06L17.25 9.95929Z" fill="white"/>
          </svg>

        </div>
        <div class="icon-wrap mute-unmute">
          <svg class="icon unmuted toggled" width="25" height="18" viewBox="0 0 25 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg class="icon transcript" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
...
async function onEnter() {
  ...
  frontEl.value.classList.toggle('visible')
  await Promise.all([
...
const playPause = () => {
  if(vidEl.value) {
    if(checkPlaying(vidEl.value)) { vidEl.value.pause(); }
    else { vidEl.value.play(); }
  }
}

const muteUnmute = () => {
  if(vidEl.value) vidEl.value.muted = !vidEl.value.muted;
}


const restartVideo = () => { console.log('restarting'); if(vidEl.value) vidEl.value.currentTime = 0; }
...
onMounted(() => {
  ...
  if (taglineEl.value) {
    ...
    playPauseEl?.addEventListener('pointerdown', () => {
      playPause();
      Array.from(playPauseEl.children).forEach(icon => {
        icon.classList.toggle('toggled');
      })
    })
    muteUnmuteEl?.addEventListener('pointerdown', () => {
      muteUnmute();
      Array.from(muteUnmuteEl.children).forEach(icon => {
        icon.classList.toggle('toggled');
      })
    })
